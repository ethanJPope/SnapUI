...
public class SnapUIHierarchyWindow : EditorWindow
{
    ...
    private void HandleDeleteKey(Canvas targetCanvas)
    {
        Event e = Event.current;
        ...
        if (!IsUnderCanvas(targetCanvas, go)) return;

        RectTransform rootRect = targetCanvas.GetComponent<RectTransform>();
        if (rootRect != null && go == rootRect.gameObject)
        {
            return;
        }
    ...
    private bool IsUnderCanvas(Canvas canvas, GameObject go)
    {
        if (canvas == null || go == null) return false;

        Canvas parentCanvas = go.GetComponentInParent<Canvas>();
        return parentCanvas == canvas;
    }
}
